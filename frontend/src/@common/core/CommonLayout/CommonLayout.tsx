...
import {
	generateSettings,
	selectCommonCurrentSettings,
	selectCommonDefaultSettings,
	setSettings,
	commonSettingsSlice
} from '@common/core/CommonSettings/store/commonSettingsSlice';
...
export type CommonRouteObjectType = RouteObject & {
	settings?: CommonSettingsConfigType;
};

export type CommonRouteMatchType = RouteMatch & {
	route: CommonRouteObjectType;
};
...
function CommonLayout(props: CommonLayoutProps) {
	...
	const defaultSettings = useSelector(selectCommonDefaultSettings);
	...
	const matchedRoutes = matchRoutes(routes, pathname) as CommonRouteMatchType[] | null;

	const matched = matchedRoutes?.[0] || false;

	const newSettings = useRef<PartialDeep<CommonSettingsConfigType>>(settings);

	const shouldAwaitRender = useCallback(() => {
		let _newSettings: CommonSettingsConfigType;
		...
		if (typeof matched !== 'boolean') {
			...
			_newSettings = newSettings.current as CommonSettingsConfigType;
		}
	...
	useDeepCompareEffect(() => {
		if (!_.isEqual(currentSettings, settings)) {
			dispatch(setSettings(currentSettings as CommonSettingsConfigType));
		}
...
export default withSlices<CommonLayoutProps>([commonSettingsSlice])(CommonLayout);
