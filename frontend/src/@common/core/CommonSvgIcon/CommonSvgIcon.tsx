...
type CommonSvgIconProps = BoxProps & {
	fill?: string;
	xmlns?: string;
	viewBox?: string;
	size?: number | string;
...
/**
 * The Root styled component is used to style the root div of the CommonSvgIcon component.
 * It uses the styled function from the MUI styles library to create a styled component.
 */
const Root = styled(Box)<CommonSvgIconProps>(({ theme, size = 24, color = 'inherit' }) => ({
	width: size,
	height: size,
	minWidth: size,
	minHeight: size,
...
/**
 * The CommonSvgIcon component is responsible for rendering an SVG icon with a specified size and color.
 * It uses various MUI components to render the icon.
 * The component is memoized to prevent unnecessary re-renders.
 */
const CommonSvgIcon = forwardRef<SVGSVGElement, CommonSvgIconProps>((props, ref) => {
