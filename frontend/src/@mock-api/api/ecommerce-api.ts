...
export const eCommerceApiMocks = (mock: ExtendedMockAdapter) => {
	...
	mock.onPost('/ecommerce/products').reply(({ data }) => {
		const newProduct = { id: CommonUtils.generateGUID(), ...JSON.parse(data as string) } as EcommerceProduct;
	...
	mock.onPost('/ecommerce/orders').reply((config) => {
		const newOrder = { id: CommonUtils.generateGUID(), ...JSON.parse(config.data as string) } as EcommerceOrder;
