...
export const notesApiMocks = (mock: ExtendedMockAdapter) => {
	...
	mock.onPost('/notes').reply(({ data }) => {
		const newNote = { id: CommonUtils.generateGUID(), ...JSON.parse(data as string) } as NotesNote;
	...
	mock.onPost('/notes/labels').reply(({ data }) => {
		const newLabel = { id: CommonUtils.generateGUID(), ...JSON.parse(data as string) } as NotesLabel;
