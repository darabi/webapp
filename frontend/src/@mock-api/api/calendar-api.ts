...
export const calendarApiMocks = (mock: ExtendedMockAdapter) => {
	...
	mock.onPost('/calendar/labels').reply(({ data }) => {
		const newLabel = { id: CommonUtils.generateGUID(), ...JSON.parse(data as string) } as Label;
	...
	mock.onPost('/calendar/events').reply(({ data }) => {
		const newEvent = { id: CommonUtils.generateGUID(), ...JSON.parse(data as string) } as Event;
